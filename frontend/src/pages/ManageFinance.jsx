...
export default function ManageFinance() {
  ...
  const handleCopyInviteLink = () => {
    ...
    setCopied(true);
    setSuccess('Invite link copied to clipboard! Anyone with this link can request Finance Team access.');
  ...
  const handleApproveFinance = async (finUser) => {
    ...
    setSuccess('');
    try {
      await request(`/api/auth/users/${finUser.id}/approve`, {
        method: 'POST',
        token,
      });
      setSuccess(`Finance Team member "${finUser.name || finUser.email}" approved and activated successfully.`);
      load();
    } catch (err) {
    ...
    try {
      await request('/api/auth/users', {
        ...
        body: {
          ...
          department: form.department?.trim() || 'Finance & Accounts',
        },
      });
      ...
      setError(err.message || 'Failed to create finance account');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to update finance account');
    } finally {
      setEditing(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="bg-white border border-gray-200/90 rounded-2xl p-6 sm:p-7 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 font-normal mt-1 max-w-2xl">
            Manage Finance & Accounting accounts in the {user?.tenant_name || 'company'} workspace. Finance members oversee invoice settlements, billing schedule releases, and expense disbursements.
          </p>
          ...
          <div className="flex items-center gap-2 mt-4 flex-wrap">
            ...
            <span className="px-3 py-1 rounded-full bg-white border border-gray-200 text-gray-800 text-xs font-semibold shadow-2xs">
              {financeUsers.length} active finance members
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0 flex-wrap">
          <button
            ...
            title="Copy public invite link for new Finance Members"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
        <div className="bg-white border border-gray-200/90 rounded-2xl p-5 shadow-xs flex flex-col justify-between">
          ...
          <div className="text-xs text-gray-500 font-medium">
            Registered accounting officers
          </div>
        </div>
      ...
      {/* Finance Data Table Card */}
      <div className="bg-white border border-gray-200/90 rounded-2xl p-5 sm:p-6 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-base font-bold text-gray-900 tracking-tight">Active Finance Members</h2>
            <p className="text-xs text-gray-500">All finance credentials provisioned under {user?.tenant_name}</p>
          </div>
        ...
        {loading ? (
          <div className="py-12 text-center text-xs text-gray-400">Loading finance accounts...</div>
        ) : filteredFinance.length === 0 ? (
          <div className="py-12 text-center text-xs text-gray-400">
            No finance accounts found matching your query.
          </div>
        ) : (
          <div
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredFinance.map((u) => (
                  <tr key={u.id} className="hover:bg-gray-50/60 transition-colors">
                    ...
                    {/* Department */}
                    <td className="py-3.5 px-3 text-gray-500 font-medium text-[11px]">
                      {u.department || 'Finance & Accounts'}
                    </td>
                    ...
                    <td className="py-3.5 px-3 text-right">
                      {u.is_active === false ? (
                        <div className="inline-flex items-center gap-2.5">
                          <button
                            type="button"
                            onClick={() => handleApproveFinance(u)}
      ...
      {/* Create Finance Member Modal Popup */}
      {showCreateModal && (
        <div
            ...
            <form onSubmit={handleCreateFinance} className="space-y-3.5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInput}
                  placeholder="e.g. Finance, Accounting, Invoicing"
                  className="w-full px-3.5 py-2 text-xs text-gray-900 bg-white border border-gray-200 rounded-lg focus:outline-hidden focus:ring-1 focus:ring-black transition-all"
                />
              </div>
