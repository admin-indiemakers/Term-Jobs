...
export default function ConfigureVendorAccounts() {
  const { token } = useAuth();
  ...
  const load = () => {
    ...
    request('/api/auth/users', { token })
      .then((usersRes) => {
        setRecruiters((usersRes || []).filter((u) => u.role === 'Recruiter'));
        setError('');
      })
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      setSuccess(`Recruiter account ${confirmDelete.email} deleted.`);
  ...
  return (
    <div className="page">
      <WelcomeBanner
        title="Configure Vendor Accounts"
        subtitle="View and manage Recruiter accounts across all consultancy (vendor) companies. Recruiters are provisioned by each vendor's Admin."
      >
      ...
      <div className="glass-panel table-card">
        <div className="table-head">
          <div>
            <h2 className="card-title">Vendor Recruiter Accounts</h2>
            <p className="muted" style={{ fontSize: '0.82rem' }}>{recruiters.length} total</p>
          </div>
        </div>
        {loading ? (
          ...
          <p className="muted" style={{ padding: 16 }}>
            ...
            <button
              ...
              onClick={() => navigate('/dashboard/superadmin/onboard-vendor')}
            >
              Onboard a vendor company
            </button>
          </p>
        ) : (
      ...
      {confirmDelete && (
        <div className="modal-overlay" onClick={() => setConfirmDelete(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <h3 className="modal-title">Delete Recruiter account?</h3>
      ...
      {edit && (
        <div className="modal-overlay" onClick={() => setEdit(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <h3 className="modal-title">Edit Recruiter Account</h3>
