...
import {
  Flag, Check, ArrowRight, AlertCircle, Laptop, BookOpen, CheckCircle2, RefreshCw, ShieldCheck
} from 'lucide-react';
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-[#F2F2EE]">
              ...
              {activationGates.filter((g) => g.type === 'blocking' && g.status !== 'cleared').length === 0 && (
                <button
                  type="button"
                  disabled={activatingWorkOrder}
                  ...
                  className="px-5 py-2 text-[12px] font-bold hover:bg-[#15803D] cursor-pointer disabled:opacity-50 flex items-center gap-1.5 shadow-2xs"
                >
                  {activatingWorkOrder ? 'Activating...' : 'Activate Work Order'}
                </button>
              )}
            </div>
          </div>
